 ...
 * 节点管理器
 */
export class DesignerNodeManager {
    ...
    createLoopContainer(x, y) {
        ...
        // 生成唯一ID
        const containerId = `loop_container_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    generatePropertyForm(cell, config) {
        ...
        let formHtml = `
            <div class="form-group">
                <label>节点类型</label>
                <input type="text" value="${nodeConfig.name || nodeType}" readonly>
            </div>
            <div class="form-group">
                <label>节点名称</label>
                <input type="text" id="nodeName" value="${config.name || ''}" placeholder="输入节点名称">
            </div>
        `;
        ...
        formHtml += `
            <div class="form-group">
                <label>超时时间 (毫秒)</label>
                <input type="number" id="timeout" value="${config.timeout || 10000}" min="1000" max="60000">
            </div>
            <div class="form-group">
                <label>延迟时间 (毫秒)</label>
                <input type="number" id="delay" value="${config.delay || 0}" min="0" max="10000">
            </div>
        `;
        ...
        return `
            <div class="form-group">
                ...
                <select id="locatorStrategy">
                    ...
                    <option value="class" ${config.locator?.strategy === 'class' ? 'selected' : ''}>Class</option>
                    <option value="text" ${config.locator?.strategy === 'text' ? 'selected' : ''}>文本内容</option>
                </select>
            </div>
            <div class="form-group">
                <label>定位值</label>
                <input type="text" id="locatorValue" value="${config.locator?.value || ''}" placeholder="输入定位值">
            </div>
        `;
    }
    ...
    generateInputForm(config) {
        return `
            ...
            <div class="form-group">
                <label>输入文本</label>
                <textarea id="inputText" placeholder="输入要填写的文本">${config.inputText || ''}</textarea>
            </div>
        `;
    }
    ...
    generateWaitForm(config) {
        return `
            <div class="form-group">
                <label>等待时间 (毫秒)</label>
                <input type="number" id="waitTime" value="${config.waitTime || 1000}" min="100" max="30000">
            </div>
        `;
    }
    ...
    generateSmartWaitForm(config) {
        return `
            <div class="form-group">
                <label>等待类型</label>
                <select id="waitType">
                    <option value="element" ${config.waitType === 'element' ? 'selected' : ''}>等待元素出现</option>
                    <option value="disappear" ${config.waitType === 'disappear' ? 'selected' : ''}>等待元素消失</option>
                    <option value="clickable" ${config.waitType === 'clickable' ? 'selected' : ''}>等待元素可点击</option>
                    <option value="visible" ${config.waitType === 'visible' ? 'selected' : ''}>等待元素可见</option>
                </select>
            </div>
            <div class="form-group">
                <label>定位策略</label>
        ...
        return `
            <div class="form-group">
                ...
                <select id="conditionType">
                    <option value="element" ${config.conditionType === 'element' ? 'selected' : ''}>元素存在</option>
                    <option value="attribute" ${config.conditionType === 'attribute' ? 'selected' : ''}>属性值判断</option>
                    <option value="text" ${config.conditionType === 'text' ? 'selected' : ''}>文本内容判断</option>
                    <option value="url" ${config.conditionType === 'url' ? 'selected' : ''}>URL判断</option>
                </select>
            </div>
            <div class="form-group">
                <label>定位策略</label>
            ...
            <div class="form-group" id="attributeGroup" style="display: ${config.conditionType === 'attribute' ? 'block' : 'none'}">
                <label>属性名称</label>
                <input type="text" id="attributeName" value="${config.attributeName || ''}" placeholder="如: class, id, value">
            </div>
            <div class="form-group">
                <label>比较方式</label>
            ...
            <div class="form-group">
                <label>期望值</label>
                <input type="text" id="expectedValue" value="${config.expectedValue || ''}" placeholder="输入期望的值">
            </div>
        `;
    }
    ...
    generateLoopForm(config) {
        let formHtml = `
            <div class="form-group">
                <label>循环类型</label>
                <select id="loopType" ${config.loopType === 'container' ? 'disabled' : ''}>
                    <option value="element" ${config.loopType === 'element' ? 'selected' : ''}>元素循环</option>
                    <option value="count" ${config.loopType === 'count' ? 'selected' : ''}>次数循环</option>
                    <option value="container" ${config.loopType === 'container' ? 'selected' : ''}>循环容器</option>
                </select>
            </div>
        `;
        ...
        if (config.loopType === 'element') {
            formHtml += `
                <div class="form-group">
                    <label>元素选择器</label>
                    <input type="text" id="elementSelector" value="${config.elementSelector || ''}" placeholder="输入元素列表的CSS选择器">
                </div>
            `;
        }
        ...
        formHtml += `
            <div class="form-group">
                <label>最大循环次数</label>
                <input type="number" id="maxIterations" value="${config.maxIterations || 10}" min="1" max="1000">
            </div>
        `;
        ...
        return `
            <div class="form-group">
                ...
                <select id="extractType">
                    <option value="text" ${config.extractType === 'text' ? 'selected' : ''}>文本内容</option>
                    <option value="attribute" ${config.extractType === 'attribute' ? 'selected' : ''}>属性值</option>
                    <option value="html" ${config.extractType === 'html' ? 'selected' : ''}>HTML内容</option>
                </select>
            </div>
            <div class="form-group">
                <label>定位策略</label>
            ...
            <div class="form-group" id="attributeGroup" style="display: ${config.extractType === 'attribute' ? 'block' : 'none'}">
                <label>属性名称</label>
                <input type="text" id="attributeName" value="${config.attributeName || ''}" placeholder="如: href, src, value">
            </div>
            <div class="form-group">
                <label>变量名称</label>
                <input type="text" id="variableName" value="${config.variableName || ''}" placeholder="保存提取结果的变量名">
            </div>
        `;
    }
        ...
        return `
            <div class="form-group">
                ...
                <select id="checkType">
                    ...
                    <option value="selected" ${config.checkType === 'selected' ? 'selected' : ''}>元素选中</option>
                </select>
            </div>
            <div class="form-group">
                <label>定位策略</label>
    ...
    bindPropertyFormEvents(cell) {
        ...
        if (conditionType) {
            conditionType.onchange = () => {
                ...
                if (attributeGroup) {
                    attributeGroup.style.display = conditionType.value === 'attribute' ? 'block' : 'none';
                }
            };
        }
        ...
        if (extractType) {
            extractType.onchange = () => {
                const attributeGroup = document.getElementById('attributeGroup');
                if (attributeGroup) {
                    attributeGroup.style.display = extractType.value === 'attribute' ? 'block' : 'none';
                }
            };
        }
    }
    ...
    updateNodeDisplay(cell, config) {
        // 更新节点标签
        const label = config.name || this.designer.nodeTypes[config.type]?.name || config.type;
    ...
    async testLocator(cell) {
        ...
        try {
            ...
            if (typeof testLocator === 'function') {
                ...
                this.designer.updateStatus(result.success ? '定位测试成功' : '定位测试失败: ' + result.error);
            } else {
