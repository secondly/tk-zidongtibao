...
class MxGraphWorkflowDesigner {
  constructor() {
    ...
    this.currentDisplayedCell = null; // 当前显示在属性面板中的节点
  ...
  async addNodeToCanvas(nodeType, x = 100, y = 100, parentContainer = null) {
    ...
    if (nodeType === "loop") {
      ...
      const nodeData = {
        ...
        name: config.name,
        loopType: loopType,
        locator: { strategy: "css", value: "" }, // 添加默认locator配置
        loopSelector: "",
        maxIterations: 10,
        startIndex: 0,
        endIndex: -1,
  ...
  showPropertyPanel(cell) {
    ...
    if (this.currentDisplayedCell && this.currentDisplayedCell.id === cell.id) {
    ...
    if (this.currentDisplayedCell && this.currentDisplayedCell.id !== cell.id) {
      console.log(
        `🔧 [DEBUG] 切换节点前保存配置: ${this.currentDisplayedCell.id} -> ${cell.id}`
      );
      this.saveNodeConfig(this.currentDisplayedCell);
    }
    ...
    // 记录当前显示的节点
    this.currentDisplayedCell = cell;
  ...
  hidePropertyPanel() {
    // 隐藏面板前保存当前配置
    if (this.currentDisplayedCell) {
      console.log(
        `🔧 [DEBUG] 隐藏面板前保存配置: ${this.currentDisplayedCell.id}`
      );
      this.saveNodeConfig(this.currentDisplayedCell);
    ...
    try {
      ...
      if (this.editMode && this.originalWorkflow) {
        ...
        workflowData.createdAt =
          this.originalWorkflow.createdAt || new Date().toISOString();
        ...
        this.updateStatus(
          `✅ 工作流 "${finalName}" 编辑完成！请关闭设计器窗口以应用更改。`
        );
        ...
        formHtml += `
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" id="checkType">
                            ...
                            <option value="disabled" ${
                              config.checkType === "disabled" ? "selected" : ""
                            }>元素是否禁用</option>
                            <option value="checked" ${
                              config.checkType === "checked" ? "selected" : ""
                            }>复选框是否选中</option>
                            <option value="unchecked" ${
                              config.checkType === "unchecked" ? "selected" : ""
                            }>复选框是否未选中</option>
                            <option value="hasText" ${
                              config.checkType === "hasText" ? "selected" : ""
                            }>元素是否包含文本</option>
                            <option value="hasClass" ${
                              config.checkType === "hasClass" ? "selected" : ""
                            }>元素是否包含CSS类</option>
                            <option value="hasAttribute" ${
                              config.checkType === "hasAttribute"
                                ? "selected"
                                : ""
                            }>元素是否包含属性</option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" id="resultAction">
                            <option value="continue" ${
                              config.resultAction === "continue"
                                ? "selected"
                                : ""
                            }>无论结果都继续</option>
                            <option value="stopIfTrue" ${
                              config.resultAction === "stopIfTrue"
                                ? "selected"
                                : ""
                            }>条件满足时停止</option>
                            <option value="stopIfFalse" ${
                              config.resultAction === "stopIfFalse"
                                ? "selected"
                                : ""
                            }>条件不满足时停止</option>
                        </select>
        ...
        formHtml += `
                        ...
                        <select class="form-select" id="locatorType">
                            <option value="css" ${
                              ...
                              config.locator?.type === "css"
                                ? "selected"
                                : ""
                            }>CSS选择器 [示例: .price, #title]</option>
                            <option value="xpath" ${
                              config.locator?.strategy === "xpath" ||
                              config.locator?.type === "xpath"
                                ? "selected"
                                : ""
                            }>XPath [示例: //span[@class='price']]</option>
                            <option value="id" ${
                              config.locator?.strategy === "id" ||
                              config.locator?.type === "id"
                                ? "selected"
                                : ""
                            }>ID [示例: product-title]</option>
                            <option value="className" ${
                              config.locator?.strategy === "className" ||
                              config.locator?.type === "className"
                                ? "selected"
                                : ""
                            }>类名 [示例: product-price]</option>
                            <option value="text" ${
                              config.locator?.strategy === "text" ||
                              config.locator?.type === "text"
                                ? "selected"
                                : ""
                            }>文本内容 [示例: 价格, 标题]</option>
                            <option value="contains" ${
                              config.locator?.strategy === "contains" ||
                              config.locator?.type === "contains"
                                ? "selected"
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" id="extractAttribute">
                            <option value="text" ${
                              config.extractAttribute === "text"
                                ? "selected"
                                : ""
                            }>文本内容</option>
                            <option value="innerText" ${
                              config.extractAttribute === "innerText"
                                ? "selected"
                                : ""
                            }>纯文本内容</option>
                            <option value="innerHTML" ${
                              config.extractAttribute === "innerHTML"
                                ? "selected"
                                : ""
                            }>HTML内容</option>
                            <option value="href" ${
                              config.extractAttribute === "href"
                                ? "selected"
                                : ""
                            }>链接地址</option>
                            <option value="src" ${
                              config.extractAttribute === "src"
                                ? "selected"
                                : ""
                            }>图片地址</option>
                            <option value="value" ${
                              config.extractAttribute === "value"
                                ? "selected"
                                : ""
                            }>输入值</option>
                            <option value="title" ${
                              config.extractAttribute === "title"
                                ? "selected"
                                : ""
                            }>标题属性</option>
                            <option value="alt" ${
                              config.extractAttribute === "alt"
                                ? "selected"
                                : ""
                            }>替代文本</option>
                            <option value="data-*" ${
                              config.extractAttribute === "data-*"
                                ? "selected"
                                : ""
                            }>自定义数据属性</option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input" id="customAttribute" value="${
                          config.customAttribute || ""
                        }" placeholder="当选择自定义数据属性时填写">
                        <div class="form-help">例如：data-price, data-id 等</div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">数据存储变量</label>
                        <input type="text" class="form-input" id="variableName" value="${
                          config.variableName || ""
                        }" placeholder="存储提取数据的变量名">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" id="extractMultiple">
                            <option value="false" ${
                              config.extractMultiple === false ? "selected" : ""
                            }>只提取第一个</option>
                            <option value="true" ${
                              config.extractMultiple === true ? "selected" : ""
                            }>提取所有匹配的</option>
                        </select>
                    </div>
                `;
        ...
        formHtml += `
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" id="locatorStrategy">
                            <option value="css" ${
                              config.locator?.strategy === "css" ||
                              config.locatorStrategy === "css"
                                ? "selected"
                                : ""
                            }>CSS选择器 [示例: .btn-primary, #submit-btn]</option>
                            <option value="xpath" ${
                              config.locator?.strategy === "xpath" ||
                              config.locatorStrategy === "xpath"
                                ? "selected"
                                : ""
                            }>XPath [示例: //button[@class='btn']]</option>
                            <option value="id" ${
                              config.locator?.strategy === "id" ||
                              config.locatorStrategy === "id"
                                ? "selected"
                                : ""
                            }>ID [示例: submit-button]</option>
                            <option value="className" ${
                              config.locator?.strategy === "className" ||
                              config.locatorStrategy === "className"
                                ? "selected"
                                : ""
                            }>类名 [示例: btn-primary]</option>
                            <option value="text" ${
                              config.locator?.strategy === "text" ||
                              config.locatorStrategy === "text"
                                ? "selected"
                                : ""
                            }>文本内容 [示例: 确定, 提交]</option>
                            <option value="contains" ${
                              config.locator?.strategy === "contains" ||
                              config.locatorStrategy === "contains"
                                ? "selected"
                                : ""
                            }>包含文本 [示例: 部分文本匹配]</option>
                            <option value="tagName" ${
                              config.locator?.strategy === "tagName" ||
                              config.locatorStrategy === "tagName"
                                ? "selected"
                                : ""
                            }>标签名 [示例: button, input]</option>
                        </select>
                    ...
                    <div class="form-group">
                        <label class="form-label">定位值</label>
                        <input type="text" class="form-input" id="locatorValue" value="${
                          config.locator?.value || config.locatorValue || ""
                        }" placeholder="输入定位值">
                        <button type="button" class="test-locator-btn" style="margin-left: 10px; padding: 5px 10px; background: #007bff; color: white; border: none; border-radius: 3px;">🎯 测试</button>
                        <div class="form-help">用于定位页面元素的值</div>
                    </div>
        ...
        formHtml += `
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" id="conditionType">
                            ...
                            <option value="class" ${
                              config.conditionType === "class" ? "selected" : ""
                            }>CSS类名</option>
                            <option value="style" ${
                              config.conditionType === "style" ? "selected" : ""
                            }>样式属性</option>
                            <option value="value" ${
                              config.conditionType === "value" ? "selected" : ""
                            }>输入框值</option>
                            <option value="exists" ${
                              config.conditionType === "exists"
                                ? "selected"
                                : ""
                            }>元素存在</option>
                            <option value="visible" ${
                    ...
                    <div class="form-group" id="attributeNameGroup" style="display: ${
                      ...
                      config.conditionType === "style"
                        ...
                        <input type="text" class="form-input" id="attributeName" value="${
                          config.attributeName || ""
                        }" placeholder="例如: disabled, data-status, color">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" id="comparisonType">
                            ...
                            <option value="contains" ${
                              config.comparisonType === "contains"
                                ? "selected"
                                : ""
                            }>包含</option>
                            <option value="notContains" ${
                              config.comparisonType === "notContains"
                                ? "selected"
                                : ""
                            }>不包含</option>
                            <option value="startsWith" ${
                              config.comparisonType === "startsWith"
                                ? "selected"
                                : ""
                            }>开始于</option>
                            <option value="endsWith" ${
                              config.comparisonType === "endsWith"
                                ? "selected"
                                : ""
                            }>结束于</option>
                            <option value="isEmpty" ${
                              config.comparisonType === "isEmpty"
                                ? "selected"
                                : ""
                            }>为空</option>
                            <option value="isNotEmpty" ${
                              config.comparisonType === "isNotEmpty"
                                ? "selected"
                                : ""
                            }>不为空</option>
                            <option value="hasAttribute" ${
                              config.comparisonType === "hasAttribute"
                                ? "selected"
                                : ""
                            }>具有属性</option>
                            <option value="notHasAttribute" ${
                              config.comparisonType === "notHasAttribute"
                                ? "selected"
                                : ""
                            }>不具有属性</option>
                        </select>
                    ...
                    <div class="form-group" id="expectedValueGroup" style="display: ${
                      [
                        ...
                        "hasAttribute",
                        "notHasAttribute",
                      ].includes(config.comparisonType)
                        ? "none"
                        : "block"
                    };">
                        <label class="form-label">期望值</label>
  ...
  saveNodeConfig(cell) {
    ...
    switch (nodeType) {
      ...
      case "smartWait":
        ...
        if (smartWaitAttributeName)
          config.attributeName = smartWaitAttributeName.value;
      ...
      case "extract":
        ...
        if (extractMultiple)
          config.extractMultiple = extractMultiple.value === "true";
        break;
      ...
      case "loop":
        ...
        if (operationDelay)
          config.operationDelay = parseInt(operationDelay.value);
        break;
  ...
  exportWorkflowData() {
    try {
      // 在导出前，确保当前正在编辑的节点配置已保存
      if (this.selectedCell) {
        console.log(
          "🔧 [DEBUG] 导出前保存当前选中节点配置:",
          this.selectedCell.id
        );
        this.saveNodeConfig(this.selectedCell);
      }

      // 使用模块中的转换函数
      const workflowName =
        document.getElementById("workflowName")?.value || "未命名工作流";
    ...
    try {
      ...
      vertices.forEach((cell) => {
        if (cell.nodeData && typeof cell.nodeData === "object") {
          this.nodeConfigs.set(cell.id, cell.nodeData);
          console.log(
            `🔧 [DEBUG] 重建顶层节点配置: ${cell.id} -> ${cell.nodeData.type}`,
            cell.nodeData
          );
          ...
          console.warn(
            `🔧 [DEBUG] 节点 ${cell.id} 缺少 nodeData 或 nodeData 不是对象:`,
            cell.nodeData
          );
        }
        // 递归处理容器内的子节点
    ...
    const applyStylesToContainer = (container) => {
      ...
      children.forEach((cell) => {
        if (cell.nodeData && cell.nodeData.type) {
          ...
          console.log(
            `应用子节点样式: ${cell.id} -> ${styleName} (${nodeType})`
          );
  ...
  validateConnection(source, target) {
    ...
    if (
      this.isLastNodeInContainer(source) &&
      this.isValidTargetForContainerExit(target)
    ) {
      return true;
    }
    ...
    if (this.graph.isSwimlane(cell)) {
      ...
      if (children.length > 0) {
        ...
        const confirmed = confirm(
          `该循环容器内包含 ${children.length} 个节点，确定要删除吗？\n删除容器将同时删除其内部的所有节点。`
        );
        if (!confirmed) {
          return; // 用户取消删除
        }
  ...
  toggleConditionFields(select) {
    const attributeGroup = document.querySelector(
      ".form-group:has(#attributeName)"
    );
    const comparisonGroup = document.querySelector(
      ".form-group:has(#comparisonType)"
    );
    const expectedValueGroup = document.querySelector(
      ".form-group:has(#expectedValue)"
    );

    if (attributeGroup) {
      attributeGroup.style.display = ["attribute", "style"].includes(
        select.value
      )
        ? "block"
        : "none";
  ...
  toggleAttributeField(select) {
    ...
    if (attributeGroup) {
      attributeGroup.style.display =
        select.value === "attributeAppear" ? "block" : "none";
    }
  }
    ...
    try {
      ...
      if (!window.locatorTester) {
        if (typeof window.LocatorTester === "undefined") {
          throw new Error(
            "LocatorTester 类未加载，请确保 locatorTester.js 已正确引入"
          );
        }
      ...
      if (result.success && result.elements && result.elements.length > 0) {
        ...
        console.log("🧪 属性测试结果:", {
          ...
          attributeValue: hasAttribute
            ? element.getAttribute(attributeName.value.trim())
            : null,
        });
  ...
  async testLocator(button) {
    ...
    // 使用模块化测试器进行测试
    await this.locatorTester.testLocator(
      strategy,
      value,
      resultElement,
      button
  ...
  async showTabSelector(tabs) {
    return new Promise((resolve) => {
      ...
      document
        .getElementById("cancelTabSelect")
